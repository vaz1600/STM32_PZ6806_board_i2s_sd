...
static uint8_t sdcard_type = SDCARD_TYPE_ERR;

static uint8_t tx_ff[512] = {0xFF};

static uint8_t tx_rx_ready = 0;
...
void SDCARD_tx_rx_callback(SPI_HandleTypeDef *hspi)
{
	tx_rx_ready = 1;
}

void SDCARD_tx_callback(SPI_HandleTypeDef *hspi)
{
	tx_ready = 1;
}
...
static int SDCARD_ReadBytesDMA(uint8_t* buff, size_t buff_size) {

	tx_rx_ready = 0;
    HAL_SPI_TransmitReceive_DMA(&SDCARD_SPI_PORT, tx_ff, buff, buff_size);
...
int SDCARD_Init() {
	...
	HAL_SPI_RegisterCallback(&SDCARD_SPI_PORT, HAL_SPI_TX_RX_COMPLETE_CB_ID, SDCARD_tx_rx_callback);
	HAL_SPI_RegisterCallback(&SDCARD_SPI_PORT, HAL_SPI_TX_COMPLETE_CB_ID, SDCARD_tx_callback);
...
*/
        if(resp[0] & 0x40)
		{
			sdcard_type = SDCARD_TYPE_V2HC;
		}
		else
		{
			sdcard_type = SDCARD_TYPE_V2;
		}
    }
...
int SDCARD_ReadSingleBlock(uint32_t blockNum, uint8_t* buff) {
	...
	if(sdcard_type != SDCARD_TYPE_V2HC)
	{
		blockNum <<= 9; //ת��λ�ֽڵ�ַ
	}
...
int SDCARD_ReadData(uint8_t* buff) {
    ...
    if(SDCARD_ReadBytesDMA(buff, 512) < 0) {
        SDCARD_Unselect();
        return -2;
    }
...
int SDCARD_WriteData(const uint8_t* buff) {
    ...
    //HAL_SPI_Transmit(&SDCARD_SPI_PORT, (uint8_t*)buff, 512, HAL_MAX_DELAY);
    tx_ready = 0;
    HAL_SPI_Transmit_DMA(&SDCARD_SPI_PORT, (uint8_t*)buff, 512);
    while(tx_ready == 0);
